...
def _recursive_walk(node, registry):
    ...
    is_project = any(x in norm_path for x in ["world_model", "rl_agent", "src/", "AURA", "trellis2", "app.py", "o_voxel"])
...
def _is_user_frame(name: str) -> bool:
    """Helper to determine if a trace stack frame belongs to your actual project code."""
    if not name: return False
    norm = name.replace('\\', '/')
    
    # 1. IGNORE FIRST: Ignore the profiler itself, root modules, and library internals
    ignore_list = [
        "profiling_wrapper.py", "<module>",  # <-- Added these two!
    ...
    # 2. MATCH SECOND: If it's not ignored, is it in our project?
    project_keywords = ["trellis2", "app.py", "o_voxel", "tools/", "rl_agent", "world_model"]
...
def process_torch_trace_json(json_path: str, output_txt_path: str, top_k=50):
    ...
    # [OPTIMIZATION 1] Cache string checks so we don't recalculate the same filenames repeatedly
    user_frame_cache = {}
    ...
    for e in events:
        ...
        if stack:
            ...
            # [OPTIMIZATION 2] O(1) Inheritance: Grab the active user script from the parent
            parent_user_frame = parent['active_user_frame']
            if parent_user_frame:
                stats[name]['user_callers'][parent_user_frame] += dur

        # Is THIS current frame a user script? If so, it becomes the new active parent.
        ...
        stack.append({
            'name': name, 
            'end': end,
            'active_user_frame': active_user_frame # Pass it down to children
        })
